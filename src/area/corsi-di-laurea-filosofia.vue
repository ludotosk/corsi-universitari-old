...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea filosofia",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea di filosofia, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content: "Corsi di laurea, Corso di laurea, Corsi di laurea filosofia",
      },
    ],
    link: [
      {
        rel: "canonical",
        href: "https://www.corsiuniversitari.info/corsi-di-laurea-filosofia",
      },
    /*   {
        rel: "preload",
        href: "https://json-server-corsi.herokuapp.com/api/corsi?c=L-5&c=LM-78",
        as: "fetch",
      }, */
    ],
  },
  ...
  async beforeCreate() {
    try {
      const res = await axios.get(
        "https://json-server-corsi.herokuapp.com/api/corsi?c=L-5&c=LM-78"
      );
  ...
  /*   methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].c == "L-5" || corsi[x].c == "LM-78"  ) {
          array.push(corsi[x]);
        }
      }
      return array;
