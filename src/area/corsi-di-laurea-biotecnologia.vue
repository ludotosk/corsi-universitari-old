<template>
  <div class="container-md">
    ...
    <p>
      ...
      corso corretto. Es. <strong>biotecnologia</strong> si trova sotto
      "biotecnologia" "biotecnologia" "biotechnology" quindi per avere il
...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea biotecnologia",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea di biotecnologia, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content:
          "Corsi di laurea, Corso di laurea, Corsi di laurea biotecnologia",
      },
    ],
    link: [
      {
        rel: "canonical",
        href: "https://www.corsiuniversitari.info/corsi-di-laurea-biotecnologia",
      },
    ],
  },
  data() {
  ...
  async created(){
    try {
      const res = await axios.get('https://json-server-corsi.herokuapp.com/corsi?c=L-02&c=LM-7&c=LM-8&c=LM-9');
 ...
 /*  methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].c == "L-02" || corsi[x].c == "LM-7" || corsi[x].c == "LM-8" || corsi[x].c == "LM-9") {
          array.push(corsi[x]);
        }
      }
      return array;
