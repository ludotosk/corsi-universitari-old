...
export default {
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea di fisica, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content:
          "Corsi di laurea, Corso di laurea, Corsi di laurea fisica",
      },
    ],
    link: [
      {
  ...
  data() {
    ...
    async created(){
    try {
      const res = await axios.get('https://json-server-corsi.herokuapp.com/corsi?c=L-30&c=LM-17&c=LM-44&c=LM-79');
...
/*   methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].c == "L-30" || corsi[x].c == "LM-17" || corsi[x].c == "LM-44" || corsi[x].c == "LM-79") {
          array.push(corsi[x]);
        }
      }
      return array;
