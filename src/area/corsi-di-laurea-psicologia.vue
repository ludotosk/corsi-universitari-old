<template>
  <div class="container is-fullhd">
    ...
    <h2 class="has-text-centered is-size-3 has-text-grey has-text-left">
      Quali sono i corsi di laurea in {{ uni }}? Ecco la lista!
    </h2>
    <br />
...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea psicologia [lista]",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea di psicologia, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
    ...
    link: [
      ...
      /*  {
        rel: "preload",
        href: "https://json-corsi-fastify.herokuapp.com/corsi?c=L-24&LM-51",
        as: "fetch",
      }, */
    ],
  },
  ...
  async beforeCreate() {
    try {
      const res = await axios.get(
        "https://json-corsi-fastify.herokuapp.com/corsi?c=L-24&LM-51"
      );
  ...
  /* methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].c == "L-24" || corsi[x].c == "LM-51") {
          array.push(corsi[x]);
        }
      }
      return array;
