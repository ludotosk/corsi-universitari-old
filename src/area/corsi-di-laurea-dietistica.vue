...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea dietistica",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea di dietistica, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content:
          "Corsi di laurea, Corso di laurea, Corsi di laurea dietistica",
      },
    ],
    link: [
      {
  ...
  async created(){
    try {
      const res = await axios.get('https://json-server-corsi.herokuapp.com/corsi?c=L/SNT3&n=Dietistica (abilitante alla professione sanitaria di Dietista)');
...
/*   methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].c == "L/SNT3" && corsi[x].n.slice(0, 10) == "Dietistica") {
          array.push(corsi[x]);
        }
      }
      return array;
