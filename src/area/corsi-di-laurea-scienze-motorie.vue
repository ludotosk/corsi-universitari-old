<template>
  <div class="container is-fullhd">
    ...
    <h2 class="has-text-centered is-size-3 has-text-grey has-text-left">
      Qui puoi trovare tutti i Corsi di Laurea di {{ uni }}! [elenco completo]
    </h2>
    <br />
...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea scienze motorie [elenco]",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea di scienze motorie, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content:
          "Corsi di laurea, Corso di laurea, Corsi di laurea scienze motorie",
      },
    ],
    link: [
      {
   ...
   /*    {
        rel: "preload",
        href: "https://json-corsi-fastify.herokuapp.com/corsi?c=L-22&c=LM-67",
        as: "fetch",
      }, */
    ],
  },
  ...
  async beforeCreate() {
    try {
      const res = await axios.get(
        "https://json-corsi-fastify.herokuapp.com/corsi?c=L-22&c=LM-67"
      );
  ...
  /*   methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].c == "L-22" || corsi[x].c == "LM-67") {
          array.push(corsi[x]);
        }
      }
      return array;
