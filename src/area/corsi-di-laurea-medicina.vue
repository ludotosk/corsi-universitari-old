<template>
  <div class="container is-fullhd">
    ...
    <h2 class="has-text-centered is-size-3 has-text-dark has-text-left">
      Quali sono i corsi di laurea a {{ uni }}? Ecco l'elenco!
    </h2>
    <br />
...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea medicina [elenco]",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea di medicina, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content: "Corsi di laurea, Corso di laurea, Corsi di laurea medicina",
      },
    ],
    link: [
      {
      ...
      /*       {
        rel: "preload",
        href: "https://json-corsi-fastify.herokuapp.com/corsi?c=LM-41&c=LM-42",
        as: "fetch",
      }, */
    ],
  },
  ...
  async beforeCreate() {
    try {
      const res = await axios.get(
        "https://json-corsi-fastify.herokuapp.com/corsi?c=LM-41&c=LM-42"
      );
  ...
  /*   methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].c == "LM-41" || corsi[x].c == "LM-42") {
          array.push(corsi[x]);
        }
      }
      return array;
