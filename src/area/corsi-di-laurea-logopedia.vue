...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea logopedia",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea di logopedia, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content:
          "Corsi di laurea, Corso di laurea, Corsi di laurea logopedia",
      },
    ],
    link: [
      {
  ...
  data() {
    ...
    async created(){
    try {
      const res = await axios.get('https://json-server-corsi.herokuapp.com/corsi?c=L/SNT2&n=Logopedia (abilitante alla professione sanitaria di Logopedista)');
 ...
 /*  methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].c == "L/SNT2" && corsi[x].n.slice(0, 9) == "Logopedia") {
          array.push(corsi[x]);
        }
      }
      return array;
