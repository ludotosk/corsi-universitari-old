<template>
  <div class="container is-fullhd">
    <br />
      ...
      Corsi di laurea {{ uni }} [elenco]
    </h1>
    <h2 class="has-text-centered is-size-3 has-text-grey has-text-left">
      Quali sono i corsi di laurea di {{ uni }}? [elenco completo]
    </h2>
    <br />
...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea economia [elenco]",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea di economia, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content: "Corsi di laurea, Corso di laurea, Corsi di laurea economia",
      },
    ],
    link: [
      {
        rel: "canonical",
        href: "https://www.corsiuniversitari.info/corsi-di-laurea-economia",
      },
   /*    {
        rel: "preload",
        href: "https://json-corsi-fastify.herokuapp.com/corsi?c=L-13&c=LM-6",
        as: "fetch",
      }, */
    ],
  },
  ...
  async beforeCreate() {
    try {
      const res = await axios.get(
        "https://json-corsi-fastify.herokuapp.com/corsi?c=L-16&c=L-18&L-33&LM-56&L-63&LM-77&LM-83&LM-82&L/GASTR&LM-49&LM-56&L-41&LM-47"
      );
  ...
  /*  methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].c == "L-13" || corsi[x].c == "LM-6") {
          array.push(corsi[x]);
        }
      }
      return array;
