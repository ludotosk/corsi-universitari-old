    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea Unimi, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content:
          "Corsi di laurea, Corso di laurea, Corsi di laurea unimi",
      },
    ],
    link: [
      {
        rel: "canonical",
        href: "https://www.corsiuniversitari.info/corsi-di-laurea-unimi",
      },
    ],
  },
  data() {
  ...
  async beforeCreate() {
    try {
      const res = await axios.get(
        'https://json-server-corsi.herokuapp.com/api/corsi?u=Università degli Studi di MILANO'
      );
...
/*   methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].u == "Università degli Studi di MILANO") {
