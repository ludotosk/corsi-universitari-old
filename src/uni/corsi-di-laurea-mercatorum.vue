...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea Mercatorum",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea mercatorum, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content:
          "Corsi di laurea, Corso di laurea, Corsi di laurea mercatorum",
      },
    ],
    link: [
      {
        rel: "canonical",
        href: "https://www.corsiuniversitari.info/corsi-di-laurea-mercatorum",
      },
    ],
  },
  data() {
   ...
   async beforeCreate() {
    try {
      const res = await axios.get(
        'https://json-server-corsi.herokuapp.com/api/corsi?u=Università Telematica "Universitas MERCATORUM"'
      );
...
/*   methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].u == "Università Telematica \"Universitas MERCATORUM\"") {
          array.push(corsi[x]);
        }
      }
      return array;
