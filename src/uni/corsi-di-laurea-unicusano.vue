...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea Unicusano [lista]",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea unicusano, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content: "Corsi di laurea, Corso di laurea, Corsi di laurea unicusano",
      },
    ],
    link: [
      {
        rel: "canonical",
        href: "https://www.corsiuniversitari.info/corsi-di-laurea-unicusano",
      },
    ],
  },
  data() {
  ...
  async beforeCreate() {
    try {
      const res = await axios.get(
        "https://www.corsiuniversitari.info/api/corsi?u=UNICUSANO Università degli Studi Niccolò Cusano -Telematica Roma"
      );
  ...
  /*   methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].u == "UNICUSANO Università degli Studi Niccolò Cusano -Telematica Roma") {
          array.push(corsi[x]);
        }
      }
      return array;
