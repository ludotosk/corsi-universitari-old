...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea unifi [2021]",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea unifi, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content:
          "Corsi di laurea, Corso di laurea, Corsi di laurea unifi",
      },
    ],
    link: [
      {
        rel: "canonical",
        href: "https://www.corsiuniversitari.info/corsi-di-laurea-unifi",
      },
    ],
  },
  data() {
  ...
  async beforeCreate() {
    try {
      const res = await axios.get(
        'https://json-corsi-fastify.herokuapp.com/corsi?u=Università degli Studi di FIRENZE'
      );
...
/*   methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].u == "Università degli Studi di FIRENZE") {
