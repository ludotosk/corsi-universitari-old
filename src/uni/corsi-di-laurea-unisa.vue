    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea Unisa, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content:
          "Corsi di laurea, Corso di laurea, Corsi di laurea unisa",
      },
    ],
    link: [
      {
  ...
  async beforeCreate() {
    try {
      const res = await axios.get(
        'https://json-server-corsi.herokuapp.com/api/corsi?u=Università degli Studi di SALERNO'
      );
...
/*   methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].u == "Università degli Studi di SALERNO") {
