<template>
  <div class="container is-fullhd">
    <br />
     <h1 class="has-text-centered is-size-2 has-text-dark has-text-left">Corsi di laurea a {{uni}} [lista]</h1>
   <h2 class="has-text-centered is-size-3 has-text-dark has-text-left">Quali sono i corsi di laurea a {{uni}}? Ecco la lista!</h2>
    <br />
...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea a Genova [lista]",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea Genova, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content:
          "Corsi di laurea, Corso di laurea, Corsi di laurea Genova",
      },
    ],
    link: [
      {
        rel: "canonical",
        href: "https://www.corsiuniversitari.info/corsi-di-laurea-genova",
      },
    ],
  },
  data() {
   ...
   async beforeCreate(){
    try {
      const res = await axios.get('https://json-corsi-fastify.herokuapp.com/corsi?s=GENOVA');
