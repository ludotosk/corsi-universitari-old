<template>
  <div class="container is-fullhd">
    <br />
     <h1 class="has-text-centered is-size-2 has-text-grey has-text-left">Corsi di laurea a {{uni}} [lista 2021]</h1>
...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea a Padova [lista 2021]",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea padova, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content:
          "Corsi di laurea, Corso di laurea, Corsi di laurea padova",
      },
    ],
    link: [
      {
  ...
  data() {
    ...
    async beforeCreate(){
    try {
      const res = await axios.get('https://json-corsi-fastify.herokuapp.com/corsi?s=PADOVA');
