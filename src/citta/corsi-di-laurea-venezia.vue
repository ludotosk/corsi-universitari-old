<template>
  <div class="container is-fullhd">
    <br />
     <h1 class="has-text-centered is-size-2 has-text-grey has-text-left">Corsi di laurea a {{uni}} [2021]</h1>
...
export default {
  metaInfo: {
    ...
    meta: [
      ...
      },
      {
        name: "keywords",
        content:
          "Corsi di laurea, Corso di laurea, Corsi di laurea Venezia",
      },
    ],
    link: [
      {
  ...
  data() {
    ...
    async beforeCreate(){
    try {
      const res = await axios.get('https://json-corsi-fastify.herokuapp.com/corsi?s=VENEZIA');
