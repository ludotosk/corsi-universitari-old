...
export default {
  metaInfo: {
    ...
    title: "Corsi di laurea magistrale",
    ...
    meta: [
      {
        name: "description",
        content:
          "In questo sito potrai trovare rapidamente tutti i corsi di laurea magistrale, tutti raggruppati su una comoda tabella.",
      },
      {
        name: "keywords",
        content:
          "Corsi di laurea, Corso di laurea, Corsi di laurea magistrali",
      },
    ],
    link: [
      {
        rel: "canonical",
        href: "https://www.corsiuniversitari.info/corsi-di-laurea-magistrale",
      },
    ],
  },
  data() {
    ...
    try {
      const res = await axios.get(
        "https://json-server-corsi.herokuapp.com/api/corsi?t=Magistrale&_sort=a,u&_order=desc,asc",
      );
...
/*   methods: {
    FiltraLista: function () {
      ...
      for (var x = 0; x < corsi.length; x++) {
        if (corsi[x].t == "Magistrale") {
          magistrale.push(corsi[x]);
        }
      }
      return magistrale;
    },
